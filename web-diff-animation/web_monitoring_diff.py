...
def is_not_html(text, headers=None, check_options='normal'):
    #error
    #https://github.com/edgi-govdata-archiving/web-monitoring-diff/blob/68a061f831edc3a64b85f0a824056bdda973cc73/web_monitoring_diff/content_type.py#L1
    #line 54
...
def compute_dmp_diff(a_text, b_text, timelimit=4):
    if (isinstance(a_text, str) and isinstance(b_text, str)):
        ...
        changes = diff(a_text, b_text, checklines=False, timelimit=timelimit, cleanup="Semantic", counts_only=False)
    elif (isinstance(a_text, bytes) and isinstance(b_text, bytes)):
        #diff_bytes method merged with diff method
        changes = diff(a_text, b_text, checklines=False, timelimit=timelimit, cleanup="Semantic", counts_only=False)
        #changes = diff(a_text, b_text, checklines=False, timelimit=timelimit, cleanup_semantic=True, counts_only=False)
    else:
...
class LocalUrlComparator:
    """
    Compares PyWB links from multiple timeframes as if they are the
    ...
    - http://localhost:8060/netarchivesuite/20220625091227/https://peterjson.medium.com/miracle-one-vulnerability-to-rule-them-all-c3aed9edeea2
    """
    matcher = re.compile(r'netarchivesuite/\d{14}(im_|js_|cs_)?/(https?://)?(www.)?')
    ...
    def compare(self, url_a, url_b):
        ...
        if match_a and match_b:
            ...
            if (url_a == url_b):
                return True
            #surt
            url_a_clean = handyurl.parse(url_a).geturl()
            url_b_clean = handyurl.parse(url_b).geturl()
            return url_a_clean == url_b_clean
        return url_a == url_b
...
def html_diff_render(a_text, b_text, a_headers=None, b_headers=None,
    ...
    #the comment header is causing an exception
    #https://github.com/edgi-govdata-archiving/web-monitoring-diff/blob/main/web_monitoring_diff/html_render_diff.py
    #line 296
    ...
    #                              treebuilder='soup', return_root=False)
    soup_old = BeautifulSoup(a_text.strip() or EMPTY_HTML, SOUP_PARSER)
    soup_new = BeautifulSoup(b_text.strip() or EMPTY_HTML, SOUP_PARSER)
...
def diff_elements(old, new, comparator, include='all'):
    ...
    def fill_element(element, diff):
        ...
        #    treebuilder='soup', return_root=False)
        parsed_diff = BeautifulSoup(f'<!doctype html>\n<html><body>{diff}</body></html>', SOUP_PARSER)
...
class DiffToken(str):
    ...
    def htmljoin(self):
        return ''.join(self.pre_tags) + str.__repr__(self)[1:-1] + ''.join(self.post_tags) + self.trailing_whitespace
...
def tokenize(html, comparator, include_hrefs=True):
    ...
    chunksodu = flatten_el(body_el, skip_tag=True, include_hrefs=include_hrefs)
    
    #oduf2 = open("dump2.txt", "wb")
    #for chodu in chunksodu:
    #    oduf2.write(str(chodu).encode('ascii', 'ignore'))
    #oduf2.write("\n\n".encode('ascii', 'ignore'))
    #oduf2.close()
...
def parse_html(html):
    ...
    #the_tree = etree.parse(StringIO(html), parser)
    #print(etree.tostring(the_tree.getroot(), pretty_print=True, method="html"))
    #return the_tree
...
def flatten_el(el, include_hrefs, skip_tag=False):
    ...
    #    eltext = str(el)
    #    start_words = split_words(eltext)
    ...
    if not skip_tag:
        ...
        if el.name == 'img':
            ...
            if 'srcset' in el.attrs:
                srcset = el.attrs['srcset']
            if 'data-srcset' in el.attrs:
                srcset = el.attrs['data-srcset']
        ...
        elif el.name in undiffable_content_tags:
            #element_source = etree.tostring(el, encoding=str, method='html')
            element_source = str(el)
            #element_source = "undiff"
            yield (TokenType.undiffable, element_source)
            return
        else:
            #print("about to call start_tag")
            #print(el)
            yield (TokenType.start_tag, start_tag(el))
    #eltext = el.renderContents().decode( 'utf-8')
    eltext = get_soup_el_text(el)
    #if el.tag in void_tags and not el.text and not len(el) and not el.tail:
    if el.name in void_tags and not eltext and not len(el.contents) and not el.next_siblings:
        return
        
    #eltext = re.sub(r"\.([A-Za-z])", r". \g<1>", eltext)
    eltext_split = eltext.split("<br/>")
    #start_words = []
    hacky_br = Tag(name = "br")
    hacky_i = 1
    for eltext_split_entry in eltext_split:
       elsplit_words = split_words(eltext_split_entry)
       for word in elsplit_words:
           yield (TokenType.word, html_escape(word))
       #yield (TokenType.word, html_escape("lesleyodu"))
       if hacky_i != len(eltext_split):
           yield (TokenType.start_tag, start_tag(hacky_br))
       #elif eltext_split_entry.strip() and '<br/>' in eltext:
       #    yield (TokenType.word, html_escape("lesleyodu"))
       #else:
       #    yield (TokenType.word, html_escape("lesleyodu"))
       hacky_i = hacky_i + 1
    ...
    #for word in start_words:
    #    yield (TokenType.word, html_escape(word))
    #elchildren = el.contents
    #elchildren = el.findChildren(recursive=False)
    elchildren = el.children
    for child in elchildren:
        if isinstance(child, Tag):
            if child.name != 'br' or '<br/>' not in eltext:
              #this strips all links.... not great... but they're not really necessary in animation - LEF 12/4/22
              if child.name != 'a' or child.name == 'a' and not eltext:
                for item in flatten_el(child, include_hrefs=include_hrefs):
                  yield item
    if el.name == 'a' and 'href' in el.attrs and el.attrs['href'] and include_hrefs:
        yield (TokenType.href, el.attrs['href'])
    if not skip_tag:
        yield (TokenType.end_tag, end_tag(el))
        #eltail = ''.join(el.next_siblings)
        eltail = get_soup_el_tail(el)
        end_words = split_words(eltail)
        for word in end_words:
            yield (TokenType.word, html_escape(word))

def get_soup_el_tail(el):
    eltail = ""
    for sib in el.next_siblings:
        if isinstance(sib, NavigableString):
           #eltail += sib
           eltail += " "
           #idk what is going on here in lxml vs beautifulsoup lefrew 2022-11-01
        elif len(eltail):
           break
    return eltail
    #return ""

def get_soup_el_text(el):
    eltext = ""
    for child in el.contents:
        if isinstance(child, NavigableString):
           eltext += child
        #bug: links not preserved
        elif child.name == 'a':
            eltext += child.text
        elif child.name == 'br':
            if len(eltext) > 0:
            ...
            #eltext += ' lesleyodu '
        #   eltext += "<br/>"
        #bug: linebreaks are not preserved
        #else:
        #   if len(eltext) and eltext[-1] != ' ':
        #       eltext += " "
    return eltext
    #return ""
...
def start_tag(el):
    ...
    """
    #print(el)
    #print(el.renderContents().decode( 'utf-8'))
    #print(el)
    #print(el.name)
    #print(el.attrs)
    #print(el.attrib.items())
    #eltag = None
    ...
    for name, value in el.attrs.items():
        ...
        elattrs[name] = val2
    return '<%s%s>' % (
        el.name, ''.join([' %s="%s"' % (name, html_escape(value, True))
                         for name, value in elattrs.items()]))

def end_tag(el):
    """ The text representation of an end tag for a tag.  Includes
    trailing whitespace when appropriate.  """
    eltail = get_soup_el_tail(el)
    if eltail and start_whitespace_re.search(eltail):
        extra = ' '
    else:
        extra = ''
    return '</%s>%s' % (el.name, extra)
...
def merge_changes(change_chunks, doc, tag_type='ins'):
    ...
    for chunk in change_chunks:
                ...
                if depth > 0:
                    if is_block:
                        ...
                        if name in current_content:
                            ...
                            # where tags aren't properly nested 
                            for nested_tag in current_content:
                                doc.append(f'</{nested_tag}>')
        ...
        # so we don't consider them "open" as part of 
        if (inline_tag and
...
def assemble_diff(html1_tokens, html2_tokens, commands, include='combined'):
    ...
    #oduf = open("dump.txt", "wb")
    #oduf.write(str(html1_tokens).encode('ascii', 'ignore'))
    #oduf.write("\n\n".encode('ascii', 'ignore'))
    #html1_tokens_dump = [x.htmljoin() for x in html1_tokens]
    #html1_tokens_dump_str = ''.join(html1_tokens_dump).replace("\\nSPACER", "\n")
    #oduf.write(html1_tokens_dump_str.encode('ascii', 'ignore'))
    #oduf.write("\n\n".encode('ascii', 'ignore'))
    #oduf.write(str(html2_tokens).encode('ascii', 'ignore'))
    #oduf.write("\n\n".encode('ascii', 'ignore'))
    #oduf.write(str(commands).encode('ascii', 'ignore'))
    #oduf.close()
    ...
    for command, i1, i2, j1, j2 in commands:
        ...
        if (command == 'delete' or command == 'replace') and include_delete:
              ...
              #if 'Starbucks' in ''.join(del_tokens):
              #I exhausted the generator
              del_tokens = expand_tokens(html1_tokens[i1:i2])
              if include_insert:
